...
它以本地音乐和歌词体验为核心，提供 MIUI / HyperOS 风格界面、逐字歌词、桌面歌词、状态栏歌词、动态封面、应用内均衡器、Monet 动态取色、在线歌词匹配、WebDAV 远程曲库、LX Music API 在线音源、音乐库统计和高度可定制的播放页体验。
...
- 支持本地媒体库与自定义文件夹扫描，提供专辑、艺术家、文件夹、流派、年份、作曲家和作词家等多维度浏览。
- 提供独立音乐库搜索页，支持歌曲、专辑、歌手、歌词、重复歌曲和全标签搜索，并支持搜索历史。
...
- 播放页支持跟手下拉关闭、动态背景、模糊背景和横屏队列封面切换；横屏封面墙顶部显示歌名 / 歌手并实时滚动歌词。
...
- 支持本地歌曲在线匹配歌词：基于 Lyrico 兼容插件，可导入 / 删除歌词源插件，匹配结果写入内嵌标签或 `.lrc`。
...
- 提供应用内均衡器，含 5 段 EQ、低音增强和环绕声。
- 本地音频标签读取使用 lyrico-audiotag 主路径，支持常见音频格式的封面、基础标签和内嵌歌词读取。
...
- 支持基础中英文界面、应用内语言切换、GitHub 更新页、应用日志、备份恢复和数据导出。
...
- **光锥音乐** — 界面设计与功能实现参考。
- 感谢 Halcyon 所使用的 Miuix、Media3、FFmpeg、Lyricon、SuperLyricApi、LyricGetter-API、lyrico-audiotag / Lyrico、TagLib、163KeyDecrypter、Kyant Backdrop、Coil、OkHttp、Reorderable、accompanist-lyrics-core、accompanist-lyrics-ui 以及其它开源项目。
